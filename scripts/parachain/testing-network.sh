...
#
# First parachain and relay chain validator will always be "light" (non-validator) nodes while
# the remaining nodes will be "full" (validators)
...
launch_configured_validator() {
    ...
    then
        launch_validator $LOCAL_CONTAINER_NAME "-p $LOCAL_PORT:30333 -p $LOCAL_RPC_PORT:9933 -p $LOCAL_WS_PORT:9944" "--rpc-cors all --rpc-methods=Unsafe --validator --unsafe-rpc-external"
        print_rotate_key $LOCAL_RPC_PORT
        delete_validator "$LOCAL_CONTAINER_NAME"
        launch_validator "$LOCAL_CONTAINER_NAME" "-p $LOCAL_PORT:30333 -p $LOCAL_RPC_PORT:9933 -p $LOCAL_WS_PORT:9944" "--validator"
    else
        launch_validator "$LOCAL_CONTAINER_NAME" "-p $LOCAL_PORT:30333 -p $LOCAL_RPC_PORT:9933 -p $LOCAL_WS_PORT:9944" "--rpc-cors=all --rpc-external --ws-external"
    fi  
}

if [[ "$VALIDATORS_NUM" -ge 0 ]];
then
    launch_configured_validator "0" false

    for idx in $(seq 1 $VALIDATORS_NUM)
    do
        launch_configured_validator "$idx" true
    done
fi
